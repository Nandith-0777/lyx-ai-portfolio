...
export function LyxChat() {
  ...
  return (
    <div className="overflow-hidden rounded-4xl border border-hairline bg-card shadow-float">
      ...
      <div ref={scrollRef} className="h-[420px] overflow-y-auto px-5 py-6 sm:px-6">
        {messages.length === 0 && !loading ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <p className="max-w-sm text-[17px] leading-relaxed text-muted-foreground">
              Ask anything about Nandith's work, skills or experience. Answers come only from his
              verified portfolio.
            </p>
          </div>
        ) : (
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {suggestedQuestions.map((q) => (
              <button
                key={q}
                onClick={() => void send(q)}
                className="rounded-full border border-hairline px-3.5 py-1.5 text-[13px] text-muted-foreground transition-colors hover:border-primary hover:text-foreground"
              >
                {q}
              </button>
            ))}
